...
export const deleteFolder = asyncHandler(async (req, res) => {
    ...
    // Hard-delete all notes inside this folder for this user, then delete the folder.
    await db.delete(notes)
        .where(and(eq(notes.folderId, Number(id)), eq(notes.userId, userId)));
    ...
    res.status(200).json({ message: "Folder and all notes inside it deleted successfully." });
});